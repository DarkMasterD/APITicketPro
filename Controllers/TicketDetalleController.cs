...
namespace APITicketPro.Controllers
{
    ...
    {
        public readonly DBTicketProContext _context;

        public TicketDetalleController(DBTicketProContext context)
        {
            _context = context;
        }
        ...
        {
            List<ticket> listaTickets = (from t in _context.ticket select t).ToList();

            if (listaTickets.Count == 0)
            {
                return NotFound(new { mensaje = "No se encontraron tickets" });
            }
        ...
        {
            int idTicket = 1; // Cambiar cuando se pase desde la vista anterior
